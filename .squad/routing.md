...
|-----------|----------|----------|
| Feature implementation | Dev | New API routes, UI components, TTS integrations |
| Bug fixes | Dev | Video pipeline errors, TTS failures, broken exports |
| TypeScript / library work | Dev | `src/lib/`, `src/app/api/`, `src/components/` |
| Code review | Reviewer | Review PRs, security checks, quality gates |
| Security issues | Reviewer | Auth, input validation, secrets exposure |
| Testing & QA | QA | Write tests, find edge cases, verify pipeline flows |
| Bug reproduction | QA | Reproduce and document issues before Dev fixes |
| Docker / container | DevOps | Dockerfile changes, native module issues |
| CI/CD / workflows | DevOps | GitHub Actions, squad workflows |
| Firebase / deployment | DevOps | Firebase config, environment variables, secrets |
